...
export default {
  ...
  data() {
    ...
    selectedColor: "red",
    isEditing: false,
    textContent: "좌석이 넓어서 수다 떨기 좋아요 메뉴들 퀄리티도 일정하고요~", // 초기 텍스트 내용
    showImage44: true,
    showImage45: false,
  ...
  methods: {
    ...
    image34Click() {
    this.isEditing = !this.isEditing; // 편집 모드 토글
  },
    leftstarChange(event) {
      console.log("leftstarChange 버튼 클릭됨!");
      console.log("클릭된 요소:", event.target);
    // 이미지 상태를 전환
    this.showImage44 = !this.showImage44;
    this.showImage45 = !this.showImage45;
  },

  rightstarChange(event) {
    console.log("rightstarChange 버튼 클릭됨!");
    console.log("클릭된 요소:", event.target);
    // 이미지 상태를 전환
    this.showImage46 = !this.showImage46;
    this.showImage47 = !this.showImage47;
  },
  //   image44Click(event) {
